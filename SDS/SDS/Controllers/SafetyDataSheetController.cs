...
using SDS.Services; // For HTML parsing 
...
namespace SDS.Controllers
{
    ...
    public class SafetyDataSheetController : Controller
    {
        ...
        public SafetyDataSheetController(SdsDbContext context, IAntiforgery antiforgery, ILogger<SafetyDataSheetController> logger, SdsService sdsService)
        {
            _context = context;
        ...
        public async Task<IActionResult> Create(string id = null)
        {
            ...
            if (!string.IsNullOrEmpty(id))
            {
                ...
                model = await _sdsService.GetSdsViewModelByProductIdAsync(id);
                ViewBag.IsEdit = true;
            }
            else
            {
        ...
        public async Task<IActionResult> Save([FromBody] SdsViewModel viewModel)
        {
            try
            {
                ...
                var normalizedProductCode = _sdsService.NormalizeText(viewModel.ProductCode);
                var normalizeProductName = _sdsService.NormalizeText(viewModel.ProductName);

                // Determine if this is an update or create
                bool isUpdate = !string.IsNullOrEmpty(viewModel.ProductId);
                var productId = isUpdate ? viewModel.ProductId : await _sdsService.GenerateProductIdAsync();
                ...
                try
                {
                    ...
                    if (isUpdate)
                    {
                        ...
                        var imageContentIds = new List<string>
                        {
                            ...
                            SdsContentName.ProductImage,
                            SdsContentName.LabelImage,
                            SdsContentName.ProtectiveEquipmentImage,
                            SdsContentName.HazardClassImage,
                            SdsContentName.EnvironmentalHazardsImage
                        };
                    ...
                    // Add new content
                    var sdsContents = _sdsService.MapFromViewModelToSDSContent(viewModel, productId);
                    var headerHImages = _sdsService.MapFromViewModelToHeaderHImage(viewModel, productId);

                    sdsContents = _sdsService.ApplyTimestamps(sdsContents, isUpdate);
